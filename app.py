...
# Encabezados que simulan un dispositivo móvil real (iPhone)
HEADERS_MOVIL = {
    "User-Agent": (
...
def realizar_peticion_espn(url, max_intentos=3):
  """Realiza peticiones a ESPN simulando un iPhone con reintentos automáticos."""
  for intento in range(max_intentos):
    try:
      response = requests.get(url, headers=HEADERS_MOVIL, timeout=10)
  ...
  for liga_nombre, slug in LEAGUES_ESPN_SLUGS.items():
    ...
    for entry in entries:
      ...
      team_name = team_obj.get(
          "displayName", team_obj.get("name", "Equipo Desconocido")
      )
      stats = {
...
st.set_page_config(
    page_title="Zohan Pronostic v8.0 - Dinámico ESPN",
    page_icon="⚽",
    layout="wide",
)
...
liga_sel = st.sidebar.selectbox(
    "⚽ Seleccionar Liga", list(LEAGUES_ESPN_SLUGS.keys()), key="select_liga_main"
)
...
if st.sidebar.button("🔄 Actualizar Tabla desde ESPN", type="primary"):
  with st.spinner("Conectando con ESPN simulando acceso móvil..."):
...
with tab2:
  ...
  if not datos_liga["tabla"]:
    st.info("Sincroniza primero con ESPN para ver los equipos.")
  else:
...
with tab5:
  ...
  if not datos_liga["tabla"]:
    ...
    st.markdown("---")
    with st.expander(
        "⚔️ Bloque 1: Historial General H2H (Últimos 10 Partidos)",
        expanded=True,
    ):
    ...
    st.markdown("---")
    with st.expander(
        "🔥 Bloque 2: Casillas de Últimos Enfrentamientos Directos",
        expanded=True,
    ):
      h2h_5_goles_l_list = []
      h2h_5_goles_v_list = []
      ...
      if st.button(
        ...
        if mc_prob_l >= 65.0:
          alertas_francotirador.append(
              f"🎯 **ALERTA FRANCOTIRADOR [VICTORIA LOCAL]:** Dominio absoluto"
              f" de **{p_local}** con **{mc_prob_l:.1f}%**."
          )
        if mc_prob_v >= 55.0:
          alertas_francotirador.append(
        ...
        if alertas_francotirador:
          ...
          st.info(
              "ℹ️ Ningún mercado supera el umbral estricto en esta simulación."
          )
        ...
        col_m1.metric(
            f"Victoria {p_local}",
            f"{mc_prob_l:.1f}%",
            f"Goles: {prom_sim_gl:.2f}",
        )
        col_m2.metric("Empate", f"{mc_prob_e:.1f}%")
        col_m3.metric(
            f"Victoria {p_visita}",
            f"{mc_prob_v:.1f}%",
            f"Goles: {prom_sim_gv:.2f}",
        )
